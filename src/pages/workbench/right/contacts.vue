    ...
    <div class="item"
      ...
      <template>
        <div class="warp">
          ...
          <div
              :class="item.more.length ? 'palycall' : ''"
               v-show="item.playCallToogl"
               v-if="!item.islogin"
               @click='palycall(item.more[item.urIndex],item)'>
      ...
      <template>
        <!-- 没登录，去登录中心 -->
        <div class="login" v-show='item.islogin == true'>
          <p>请登录呼叫中心</p>
          <el-button type="primary" size='mini'>去登录</el-button>
        </div>
      </template>
    </div>
...
<script>
  ...
  import { editReturnList, isItConnected } from './../data/data.js';
  export default {
    data() {
      // 验证 手机号
      ...
      return {
        ...
        invalidType:[
          {id:0,label:"接通后20s内挂断"},
          {id:1,label:"无人接听"},
          {id:2,label:"空号"},
          {id:3,label:"停机/错号"}
        ],
        EditReturnLis: editReturnList,
